...
type Dialector struct {
	*Config
	Ctx              context.Context
	PlaceHolder      string //第一个为主要的placeholder
...
func Open(dsn string) gorm.Dialector {
	placeHolder, err := utils.GetDsnQuery(dsn, "dbholder")
	if err != nil {
		panic(err.Error())
	}
	placeHolders := strings.Split(placeHolder, ",")
	if len(placeHolders) == 0 {
		panic("place holder is not found")
	}

	mainPlaceHolder := ""
	otherPlaceHolder := []string{}
	for key, item := range placeHolders {
		placeHolderCheck(item)
		if key == 0 {
			mainPlaceHolder = item
		} else {
			otherPlaceHolder = append(otherPlaceHolder, item+".")
		}
	}
	dsn = utils.RemoveDbholder(dsn)
	return &Dialector{
...
func placeHolderCheck(placeHolder string) {
	placeHolderRune := []rune(placeHolder)
	if placeHolderRune[len(placeHolder)-1] == '.' {
		panic("Incorrect placeholder should start with $, not end.")
	}
...
func (dialector Dialector) QuoteTo(writer clause.Writer, str string) {
	...
	quotePrefix := ""
	placeHolders := append(dialector.OtherPlaceHolder, dialector.PlaceHolder)
	for _, item := range placeHolders {
		if strings.ContainsAny(str, item) {
			tableSplit := strings.Split(str, item)
			if len(tableSplit) == 2 {
				if tableSplit[0] == "" {
					quotePrefix = fmt.Sprintf("`%s`.", utils.RemovePoint(item))
					str = tableSplit[1]
				} else {
					quotePrefix = fmt.Sprintf("`%s`.", tableSplit[0])
					str = tableSplit[1]
				}
			}
		}
	...
	for _, v := range []byte(str) {
		...
		default:
			if shiftDelimiter-continuousBacktick <= 0 && !underQuoted {
				writer.WriteString(quotePrefix + "`")
				underQuoted = true
